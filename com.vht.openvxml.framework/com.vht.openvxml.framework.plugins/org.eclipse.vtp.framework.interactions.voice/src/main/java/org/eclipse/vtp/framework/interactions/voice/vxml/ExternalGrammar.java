...
/**
 * The <code>ExternalGrammar</code> class represents the &lt;grammar&gt; VXML element with a
 * reference to an external grammar file.
 * 
 * @author Trip Gilman
 * @author Lonnie Pryor
 * @version 2.0
...
public class ExternalGrammar extends Grammar {
	 ...
	 * @throws NullPointerException If the specified grammar URI is <code>null</code>.
	 */
	public ExternalGrammar(String mode, String grammarURI) throws IllegalArgumentException,
	...
	public void setGrammarURI(String grammarURI) throws IllegalArgumentException,
			NullPointerException {
		if (grammarURI == null) throw new NullPointerException("grammarURI"); //$NON-NLS-1$
		if (grammarURI.length() == 0) throw new IllegalArgumentException("grammarURI"); //$NON-NLS-1$
	 ...
	 * @throws IllegalArgumentException If the specified grammar type is empty.
	 */
	public void setGrammarType(String grammarType) throws IllegalArgumentException {
		if (grammarType != null && grammarType.length() == 0) throw new IllegalArgumentException(
				"grammarType"); //$NON-NLS-1$
	...
	protected void writeAttributes(AttributesImpl attributes) {
		...
		if (grammarType != null) writeAttribute(attributes, null, null, NAME_TYPE, TYPE_CDATA,
				grammarType);
	...
	protected String detectGrammarType() {
		if (grammarURI.endsWith(FILE_EXT_GSL)) return MIME_TYPE_GSL;
		else return MIME_TYPE_SRGS;
	}
}
