...
/**
 * The <code>AudioOutput</code> class represents the &lt;audio&gt; VXML element and specifies the
 * location of an audio file to be played to the caller. An <code>AudioOutput</code> can only be
 * contained by the Prompt and Choice elements. All other output must be some combination of
 * BasicOutput or its subclasses.
 * 
 * @author Trip Gilman
 * @author Lonnie Pryor
 * @version 2.0
...
public class AudioOutput extends OutputSet {
	...
	/**
	 * Creates a new instance of <code>AudioOutput</code> with the specified audio file URI. The URI
	 * passed in may be a string literal or an expression that will evaluate to the desired string
	 * literal.
	 * 
	 ...
	 */
	public AudioOutput(String audioFileURI) throws IllegalArgumentException, NullPointerException {
	 ...
	 * @throws NullPointerException If the supplied URI is <code>null</code>.
	 */
	public void setAudioFileURI(String audioFileURI) throws IllegalArgumentException,
			NullPointerException {
		if (audioFileURI == null) { throw new NullPointerException("audioFileURI"); //$NON-NLS-1$
		}
		if (audioFileURI.length() == 0) { throw new IllegalArgumentException("audioFileURI"); //$NON-NLS-1$
		}
	...
	public void writeWidget(ContentHandler outputHandler) throws NullPointerException, SAXException {
		...
		outputHandler.startElement(NAMESPACE_URI_VXML, NAME_AUDIO, NAME_AUDIO, attributes);
		// Write the children.
