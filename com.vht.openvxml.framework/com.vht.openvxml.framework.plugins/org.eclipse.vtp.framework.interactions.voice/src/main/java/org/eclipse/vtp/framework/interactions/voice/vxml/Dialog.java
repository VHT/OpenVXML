...
/**
 * The <code>Dialog</code> class is the base class for the family of VXML elements that collect data
 * from the caller, allow the caller to select from a set of menu options, or manipulate the flow of
 * the call during processing. <br>
 * <br>
 * Certain VXML compliant platforms allow the use of proprietary properties to be applied to the
 * dialogs of a VXML document. These properties are written to the VXML document in the order they
 * are added to the <code>Dialog</code> instance.<br>
 * <br>
 * During processing of a dialog's constituent elements, events and error can be generated.
 * <code>EventHandler</code>s can be added to the dialog instance that will handle events and errors
 * not dealt with by the child's event handlers. The handlers will be written to the VXML document
...
public abstract class Dialog extends Widget implements VXMLConstants {
	...
	/**
	 * Creates a new instance of <code>Dialog</code> with the specified id and a grammar scope of
	 * Dialog.SCOPE_DIALOG. Throws an IllegalArgumentException if id argument is <code>null</code>
	...
	/**
	 * Creates a new instance of <code>Dialog</code> with the specified id and the grammar scope of
	 * <code>scope</code>. Throws an IllegalArgumentException if id argument is <code>null</code> or
	 * is an empty string. Throws IllegalArgumentException if scope argument is not
	...
	/**
	 * Sets the grammar scope of this dialog instance. Throws IllegalArgumentException if scope is
	 * not Dialog.SCOPE_DIALOG or Dialog.SCOPE_DIALOG.
	 * 
	...
	/**
	 * Adds an EventHandler to this dialog instance. The event handlers will be written to the VXML
