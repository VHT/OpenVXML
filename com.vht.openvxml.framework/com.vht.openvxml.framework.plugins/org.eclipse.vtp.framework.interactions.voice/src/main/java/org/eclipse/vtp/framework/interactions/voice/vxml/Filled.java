...
/**
 * The <code>Filled</code> class represents the &lt;filled&gt; VXML element. It defines a list of
 * actions to be performed by the VXML interpreter when data has been collected from the caller.
 * Both <code>Field</code> and <code>Form</code> instances can contain <code>Filled</code> elements.
 * An optional list of names can be defined to specify which elements this filled element is
 * interested in. The mode determines which elements of the name list must be filled before the
 * actions are performed. Setting the mode to Filled.MODE_ANY allows the execution of the filled
 * element when data is collected for any of the named elements, where as, Filled.MODE_ALL only
 * allows execution of the filled element when data has been collected for all of the named
 * elements. The default mode is Filled.MODE_ANY. If no list of named elements is provided, the list
 * defaults to all elements within the current scope. NOTE: If a filled element is added to a field
 * whose name is not contained within the name list (only if a name list is explicitly defined), the
 * filled element will never execute.
 * 
 * @author Trip Gilman
 * @author Lonnie Pryor
 * @version 2.0
...
public class Filled extends Widget implements VXMLConstants {
	...
	/**
	 * Creates a new instance of <code>Filled</code> that will execute if any of the elements in the
	 * current scope are filled.
	 */
	public Filled() {}

	/**
	 * Creates a new instance of <code>Filled</code> that will execute based on the specified
	 * <code>mode</code> when the elements in the current scope are filled.<br>
	 * <ul>
	 ...
	 * @throws IllegalArgumentException If the specified mode is not one of "any" or "all".
	 */
	...
	/**
	 * Creates a new instance of <code>Filled</code> that will execute if any of the elements named
	...
	/**
	 * Creates a new instance of <code>Filled</code> that will execute based on the specified
	 * <code>mode</code> when the elements named in the array are filled.<br>
	 * <ul>
	 * <li>Filled.FILLED_MODE_ANY - When any elements are filled</li>
	 * <li>Filled.FILLED_MODE_ALL - When all elements are filled</li>
	 * </ul>
	...
	public void removeName(String name) throws IllegalArgumentException, NullPointerException {
		if (name == null) { throw new NullPointerException("mode"); //$NON-NLS-1$
		}
		if (name.length() == 0) { throw new IllegalArgumentException("mode"); //$NON-NLS-1$
		}
		names.remove(name);
	}
	...
	/**
	 * Adds a set of conditional actions into the list of actions to perform if this filled element
	 * is executed.
	 * 
	 * @param ifClause The If clause to add.
	 * @throws NullPointerException If the specified if clause is <code>null</code>.
	 */
	...
	/**
	 * Removes a set of conditional actions from the list of actions to perform if this filled
	 * element is executed.
	 * 
	...
	public void writeWidget(ContentHandler outputHandler) throws NullPointerException, SAXException {
		...
		outputHandler.startElement(NAMESPACE_URI_VXML, NAME_FILLED, NAME_FILLED, attributes);
		// Write the children.
	...
	protected void writeAttributes(AttributesImpl attributes) {
		...
		if (!names.isEmpty()) {
			...
			writeAttribute(attributes, null, null, NAME_NAMELIST, TYPE_CDATA, nameList.toString());
		}
	}
