...
/**
 * The <code>EventHandler</code> class is the base class for the family of VXML elements that
 * process events and errors that are generated during execution of a VXML document. Each instance
 * contains a list of actions for the interpreter to perform if the event handler receives an event.
 * These actions are written to the VXML document in the order they were added. There are two
 * optional methods to govern whether the event is received by the event handler instance. A
 * conditional expression can be specified that must evaluate to true at the time the event is
 * generated for the event handler to be notified of the event. Also, an event handler can declare
 * that it only needs to be notified if it is the n<sup>th</sup> occurrence of the event by setting
 * the count field. Each of these options can be employed separately or together.
 * 
 * @author Trip Gilman
 * @author Lonnie Pryor
 * @version 2.0
...
public abstract class EventHandler extends Widget implements VXMLConstants {
	...
	/**
	 * Creates a new instance of <code>EventHandler</code> which does not employ a conditional
	 * expression or require a specific number of occurrences to receive an event.
	 */
	protected EventHandler() {}

	/**
	 * Creates a new instance of <code>EventHandler</code> that requires a specific number of
	 * occurrences of an event to receive it. It does not employ a conditional expression that must
	 * be evaluated before receiving an event.
	 * 
	 * @param count The number of occurrences of an event for this event handler to receive it.
	 */
	...
	/**
	 * Creates a new instance of <code>EventHandler</code> with a conditional expression that must
	 * evaluate to true for the event handler to receive an event. It does not require a specific
	 * number of occurrences to receive an event.
	 * 
	 * @param condition Expression that must evaluate to true for the event handler to receive the
	 *            event
	...
	/**
	 * Creates a new instance of <code>EventHandler</code> with a conditional expression that must
	 * evaluate to true for the event handler to receive an event. It also requires a specific
	 * number of occurrences of the event to receive it.
	 * 
	 * @param count The number of occurrences of an event for this event handler to receive it.
	 * @param condition Expression that must evaluate to true for the event handler to receive the
	 *            event.
	...
	/**
	 * Returns the conditional expression that must evaluate to true before this event handler can
	 * receive an event.
	 * 
	...
	public Widget[] getActionsAndIfClauses() {
		return actionsAndIfClauses.toArray(new Widget[actionsAndIfClauses.size()]);
	}
	...
	/**
	 * Sets the conditional expression that must evaluate to true for the event handler to accept an
	 * event.
	 * 
	...
	/**
	 * Adds the specified variable element to this filled handler. Variables declared at the filled
	 * level are accessible to only its child elements.
	 * 
	...
	/**
	 * Removes the specified variable element from this filled handler. Variables declared at the
	 * filled level are accessible to only its child elements.
	 * 
	...
	/**
	 * Adds the action to the list of actions to be performed by the interpreter if this event
	 * handler receives an event. The list of actions are written to the VXML document in the order
	 * they were added.
	 * 
	...
	/**
	 * Removes the action from the list of actions to be performed by the interpreter if this event
	...
	/**
	 * Inserts a conditional set of actions into the list of actions to be performed by the
	...
	public void addIfClause(If ifClause) throws NullPointerException {
		if (ifClause == null) { throw new NullPointerException("ifClause"); //$NON-NLS-1$
		}
		actionsAndIfClauses.add(ifClause);
	}

	/**
	 * Removes the conditional set of actions from the list of actions to be performed by the
	 * interpreter if this event handler receives an event.
	 * 
	...
	protected void writeAttributes(AttributesImpl attributes) {
		...
		if (count >= 0) {
			writeAttribute(attributes, null, null, NAME_COUNT, TYPE_CDATA, String.valueOf(count));
		}
	}
	 ...
	 * @param outputHandler The content handler to write to.
	 * @throws NullPointerException If the supplied content handler is <code>null</code>.
	 * @throws SAXException If the writing of one of the actions or if clauses fails.
	 */
