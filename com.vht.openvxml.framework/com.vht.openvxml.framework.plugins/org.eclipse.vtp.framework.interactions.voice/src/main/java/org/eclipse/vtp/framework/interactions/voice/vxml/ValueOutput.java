...
/**
 * The <code>ValueOutput</code> represents the &lt;value&gt; VXML element. This element causes the
 * result of the expression to be rendered to the caller with TTS.
 * 
 * @author Trip Gilman
 * @author Lonnie Pryor
 * @version 2.0
...
public class ValueOutput extends BasicOutput {
	...
	/**
	 * Creates a new instance of <code>ValueOutput</code> with the specified expression.
	 * 
	 ...
	 * @throws NullPointerException If the supplied expression is <code>null</code>.
	 */
	public ValueOutput(String expression) throws IllegalArgumentException, NullPointerException {
	...
	public void setExpression(String expression) throws IllegalArgumentException,
		...
		if (expression == null) { throw new NullPointerException("expression"); //$NON-NLS-1$
		}
		if (expression.length() == 0) { throw new IllegalArgumentException("expression"); //$NON-NLS-1$
		}
	...
	public void writeWidget(ContentHandler outputHandler) throws NullPointerException, SAXException {
		...
		outputHandler.startElement(NAMESPACE_URI_VXML, NAME_VALUE, NAME_VALUE, attributes);
	...
	protected void writeAttributes(AttributesImpl attributes) {
		writeAttribute(attributes, null, null, NAME_EVENTEXPR, TYPE_CDATA, expression);
	}
}
