...
/**
 * Implementation of a ConfigurationAdmin dictionary that can be loaded and saved to and from an XML
 * structure..
 * 
 * @author Lonnie Pryor
 */
...
public class ConfigurationDictionary extends Dictionary implements Comparator<String>, Serializable {
	...
	private static final Set RESERVED_KEYS = Collections.unmodifiableSet(new HashSet<String>(Arrays
			.asList(new String[] { Constants.SERVICE_PID, ConfigurationAdmin.SERVICE_FACTORYPID,
	...
	public static ConfigurationDictionary[] loadAll(Element configurations) {
		if (configurations == null || !"configurations".equals(configurations.getTagName())) { return null; }
	 ...
	 * @return The XML representation of the contents of the supplied dictionaries or
		...
		if (type.equals(Vector.class)) {
			...
			if (arrayProperty == null) {
				...
				if (value == null || !value.getClass().isArray()
						|| !SIMPLE_TYPES.contains(value.getClass().getComponentType())) { return null; }
	 ...
	 * @return The parsed value or <code>null</code> if the value could not be parsed.
	 */
	private static Object parse(String value, Class type) {
		if (value == null || type == null || !type.isPrimitive() && !SIMPLE_TYPES.contains(type)) { return null; }
		try {
	 ...
	 * @return The new XML element or <code>null</code> if the value was <code>null</code>.
	 */
	@SuppressWarnings("unchecked")
	private static Element encodeValue(Document document, Object value) {
		...
		if (value instanceof Vector) {
			...
			if (size > 0) {
				...
				property.appendChild(encodeValue(document, vector.toArray((Object[]) Array
	...
	private static Object cloneValue(Object input) {
		...
		if (input.getClass().equals(Vector.class)) { return new Vector((Vector) input); }
		if (!input.getClass().isArray()) { return input; }
	 ...
	 * @throws NullPointerException If the supplied configuration is <code>null</code>.
	 */
	 ...
	 * 
	 * @param configuration The configuration data to load or <code>null</code> to clear this
	 *            dictionary.
	...
	public synchronized boolean load(Element configuration) {
		...
		if (!"configuration".equals(configuration.getTagName())) { return false; }
		properties.clear();
	 ...
	 * 
	 * @param document The document to use to create the XML structure.
	 * @return The XML representation of the contents of this dictionary or <code>null</code> if the
	 *         supplied document was <code>null</code>.
	 */
	...
	public synchronized Object get(Object key) {
		if (key == null) { throw new NullPointerException("key"); //$NON-NLS-1$
		}
		if (!(key instanceof String) || ((String) key).length() == 0) { throw new IllegalArgumentException(
				"key"); //$NON-NLS-1$
		}
	...
	public synchronized Object put(Object key, Object value) {
		...
		if (!NAMES_BY_TYPE.containsKey(value.getClass())) { throw new IllegalArgumentException(
				"value"); //$NON-NLS-1$
		}
		if (RESERVED_KEYS.contains(key)) { return cloneValue(properties.get(key)); }
		...
		if (value instanceof Vector) {
			...
			if (size > 0) {
				...
				if (element != null) {
					...
					if (!SIMPLE_TYPES.contains(type)) { throw new IllegalArgumentException(
							"value[" + //$NON-NLS-1$
									index + "]"); //$NON-NLS-1$
					}
					for (++index; index < size; ++index) {
						...
						if (!type.equals(other.getClass())) { throw new IllegalArgumentException(
								"value[" + //$NON-NLS-1$
										index + "]"); //$NON-NLS-1$
						}
					}
