...
public final class ServiceDescriptor {
	 ...
	 * @throws IllegalArgumentException If the supplied scope is not one of application, session,
	 *             execution, or action.
	 * @throws IllegalArgumentException If the supplied type is not a public, concrete class with at
	 *             least one public constructor.
	 * @throws IllegalArgumentException If the same identifier name is listed more than once in the
	 ...
	 * @throws NullPointerException If the supplied identifier array or any of its elements are
	 *             <code>null</code>.
	 */
	public ServiceDescriptor(String id, String name, String scope, Class type,
		...
		if (scope == null) { throw new NullPointerException("scope"); //$NON-NLS-1$
		}
		if (type == null) { throw new NullPointerException("type"); //$NON-NLS-1$
		}
		if (!DescriptorUtils.isValidImplementation(type)) { throw new IllegalArgumentException(
				"type"); //$NON-NLS-1$
		}
		if (identifiers == null) { throw new NullPointerException("identifiers"); //$NON-NLS-1$
		}
		this.id = id;
		this.name = name;
		...
		for (int i = 0; i < identifiers.length; ++i) {
			if (identifiers[i] == null) { throw new NullPointerException("identifiers[" //$NON-NLS-1$
					+ i + "]"); //$NON-NLS-1$
			}
			if (map.containsKey(identifiers[i].getName())) { throw new IllegalArgumentException(
	 ...
	 * @throws IndexOutOfBoundsException If the supplied index is less than zero or greater than or
	...
	public IdentifierDescriptor getIdentifier(int index) throws IndexOutOfBoundsException {
		if (index < 0 || index >= identifiers.length) { throw new IndexOutOfBoundsException(String
				.valueOf(index)); }
	...
	public String toString() {
		StringBuffer buffer = new StringBuffer(getClass().getName().substring(
				...
				.append(name).append(';').append(scope).append(';').append(type).append(';');
