...
public final class IdentifierDescriptor {
	 ...
	 * @throws IllegalArgumentException If the same qualifier is listed more than once in the
	 *             supplied qualifier array.
	 * @throws NullPointerException If the supplied name is <code>null</code>.
	 * @throws NullPointerException If the supplied qualifier array or any of its elements are
	 *             <code>null</code>.
	 */
	public IdentifierDescriptor(String name, String[] qualifiers) throws IllegalArgumentException,
		...
		if (qualifiers == null) { throw new NullPointerException("qualifiers"); //$NON-NLS-1$
		}
		this.name = name;
		Set set = new HashSet();
		for (int i = 0; i < qualifiers.length; ++i) {
			if (qualifiers[i] == null) { throw new NullPointerException("qualifiers[" //$NON-NLS-1$
					+ i + "]"); //$NON-NLS-1$
			}
			if (!set.add(qualifiers[i])) { throw new IllegalArgumentException("qualifiers[" //$NON-NLS-1$
					+ i + "]"); //$NON-NLS-1$
			}
		}
	...
	public String getQualifier(int index) throws IndexOutOfBoundsException {
		if (index < 0 || index >= qualifiers.length) { throw new IndexOutOfBoundsException(String
				.valueOf(index)); }
