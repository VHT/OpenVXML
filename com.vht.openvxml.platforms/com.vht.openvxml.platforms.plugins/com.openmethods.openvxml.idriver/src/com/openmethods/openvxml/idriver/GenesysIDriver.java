...
public class GenesysIDriver implements Runnable {
		...
		try {
			...
			if (primaryPortString != null) {
				try {
					...
					System.out.println("WARN: Primary configuration port invalid ["
							+ primaryPortString + "] using default [" + DEFAULT_CONFIG_PORT + "]");
				}
			} else {
				System.out.println("INFO: Primary configuration port not supplied, using default ["
						+ DEFAULT_CONFIG_PORT + "]");
			}
			...
			if (backupServerAddress != null) {
				if (backupPortString != null) {
					try {
						...
						System.out.println("WARN: Backup configuration port invalid ["
								+ backupPortString + "] using default [" + DEFAULT_CONFIG_PORT
								+ "]");
					}
				} else {
					System.out
			...
			if (timeoutString != null) {
				try {
					...
					System.out.println("WARN: Connection timeout invalid [" + timeoutString
							+ "] using default [" + DEFAULT_TIMEOUT + "]");
				}
			} else {
				System.out.println("INFO: Connection timeout not supplied, using default ["
						+ DEFAULT_TIMEOUT + "]");
			}
			if (!iDriver.ilConnectionOpenConfigServer80(primaryServerAddress, primaryServerPort,
					(short) 0, backupServerAddress, backupServerPort, appName, new NativeLong(
							timeout))) {
		...
		while (running) {
			...
			for (PortRecord pr : copy) {
				synchronized (pr) {
					...
					if (inactivity > 90000) {
						System.out.println("INFO: Timing out port [" + pr.port + "] after "
								+ Long.toString(inactivity / 1000) + "s of inactivity");
						pr.onCall = false;
						NativeLong result = iDriver.ilSRqNoteCallEnd(IDriver.ANY_RQ,
								new NativeLong(pr.port));
	...
	public boolean startCall(long port, String callId, String dnis, String ani) {
		...
		NativeLong result = iDriver.ilSRqNoteCallStart(IDriver.ANY_RQ, new NativeLong(port),
				callId, dnis, ani, "");
		if (result.longValue() < 0) { return false; }
	...
	public String getConnId(long port) {
		...
		NativeLong result = iDriver.ilSRqGetCallInfo(IDriver.ANY_RQ, new NativeLong(port),
				IDriver.INFO_CONN_ID);
		if (result.longValue() < 1) // error occurred
		{
			System.out.println("WARN: Could not get ConnId for port [" + port + "] code: "
					+ result.longValue());
	...
	public boolean endCall(long port) {
		...
		if (result.longValue() < 0) // error occurred
		{
			System.out.println("ERROR: Could not end call on port [" + port + "] code: "
					+ result.longValue());
			return false;
		}
	...
	public String getUData(long port, String key) {
		...
		if (result.longValue() < 1) // error occurred
		{
			System.out.println("WARN: Could not get user data (" + key + ") for port [" + port
	...
	public Map<String, String> getAllUData(long port) {
		...
		if (result.longValue() < 0) // error occurred
		{
			System.out.println("WARN: Could not get user data (ALL) for port [" + port + "] code: "
					+ result.longValue());
			return null;
		}
		...
		if (value.equals("NoMatch")) { return ret; }
	...
	public boolean addUData(long port, String key, String value, boolean delete) {
		...
		PortRecord pr = activePorts.get(Long.toString(port));
		if (pr == null) {
			System.out.println("ERROR: Received set user data (" + key + ":" + value
		...
		// with last activity updated, no need to maintain lock
		NativeLong result = iDriver.ilSRqUDataAddKD(IDriver.ANY_RQ, new NativeLong(port), key,
				value);
		if (result.longValue() < 0) // error occurred
		{
			System.out.println("WARN: Could not set user data (" + key + ":" + value
	...
	public boolean addUDataList(long port, Map<String, String> pairs, boolean delete) {
		...
		PortRecord pr = activePorts.get(Long.toString(port));
		if (pr == null) {
			System.out.println("ERROR: Received set user data (" + pairs + ") on inactive port ["
					+ port + "]");
			return false;
		}
		synchronized (pr) {
		...
		NativeLong result = iDriver.ilSRqUDataAddList(IDriver.ANY_RQ, new NativeLong(port),
				listString);
		if (result.longValue() < 1) // error occurred
		{
			System.out.println("WARN: Could not set user data (" + listString + ") for port ["
	...
	private boolean waitForEstablished(long port, ByteBuffer buffer) {
		for (int i = 0; i < 25; i++) {
			...
			{ return false; }
			if (status == IDriver.CALL_STATUS_ESTABLISHED) { return true; }
			try {
				Thread.sleep(100);
