 ...
 * @author trip
 */
public class SoapServiceSelectionPropertiesPanel extends DesignElementPropertiesPanel {
	...
	public SoapServiceSelectionPropertiesPanel(String name, IDesignElement element) {
		...
		IWebserviceProjectAspect webserviceAspect = (IWebserviceProjectAspect) project
		...
		currentKey = serviceBinding.getDescriptor() + ":" + serviceBinding.getService() + ":"
				+ serviceBinding.getPort() + ":" + serviceBinding.getOperation();
		if (descriptors.size() > 0) updateSelections();
	}
	...
	public void createControls(Composite parent) {
		...
		final Section serviceSection = toolkit.createSection(mainComp, Section.TITLE_BAR);
		...
		descriptorCombo.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (!updating && currentDescriptor != null) {
					updating = true;
					IWebserviceDescriptor descriptor = (IWebserviceDescriptor) ((IStructuredSelection) descriptorCombo
		...
		serviceCombo.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (!updating && currentService != null) {
					updating = true;
					Service service = (Service) ((IStructuredSelection) serviceCombo.getSelection())
							.getFirstElement();
		...
		portCombo.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (!updating && currentPort != null) {
					updating = true;
					Port port = (Port) ((IStructuredSelection) portCombo.getSelection())
							.getFirstElement();
					if (port != currentPort) {
		...
		operationCombo.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (!updating && currentBindingOperation != null) {
					updating = true;
					SoapBindingOperation operation = (SoapBindingOperation) ((IStructuredSelection) operationCombo
							.getSelection()).getFirstElement();
	...
	private List<Service> getSoapServices(List<Service> genericServices) {
		...
		for (Service service : genericServices) {
			if (hasSoapBinding(service)) ret.add(service);
		}
		return ret;
	}
	...
	private boolean hasSoapBinding(Service service) {
		...
		for (Port port : searchPorts) {
			if (port.getBinding() instanceof SoapBinding) return true;
		}
		return false;
	}
	...
	private List<Port> getSoapPorts(List<Port> genericPorts) {
		...
		for (Port port : genericPorts) {
			if (port.getBinding() instanceof SoapBinding) ret.add(port);
		}
		return ret;
	}
	...
	private void updateSelections() {
		...
		if (currentDescriptor != null) {
			...
			if (serviceCombo != null) serviceCombo.refresh();
			...
			if (currentService != null) {
				...
				if (portCombo != null) portCombo.refresh();
				...
				if (currentPort != null) {
					...
					if (operationCombo != null) operationCombo.refresh();
					...
					if (currentBindingOperation == null && bindingOperations.size() > 0) {
						...
						serviceBinding.setOperation(currentBindingOperation.getOperation()
								.getName());
		...
		currentKey = serviceBinding.getDescriptor() + ":" + serviceBinding.getService() + ":"
				+ serviceBinding.getPort() + ":" + serviceBinding.getOperation();
		if (!oldKey.equals(currentKey)) manager.getInputDocumentStructure().clearStructure();
	...
	public class DescriptorContentLabelProvider extends BaseLabelProvider
		implements
		ILabelProvider,
		...
		public Object[] getElements(Object inputElement) {
			if (descriptors.size() > 0) return descriptors.toArray();
	...
	public class ServiceContentLabelProvider extends BaseLabelProvider
		implements
		ILabelProvider,
		IStructuredContentProvider {
		...
		public Object[] getElements(Object inputElement) {
			if (services.size() > 0) return services.toArray();
	...
	public class PortContentLabelProvider extends BaseLabelProvider
		implements
		ILabelProvider,
		IStructuredContentProvider {
		...
		public Object[] getElements(Object inputElement) {
			if (ports.size() > 0) return ports.toArray();
	...
	public class OperationContentLabelProvider extends BaseLabelProvider
		implements
		ILabelProvider,
		IStructuredContentProvider {
		...
		public Object[] getElements(Object inputElement) {
			if (bindingOperations.size() > 0) return bindingOperations.toArray();
