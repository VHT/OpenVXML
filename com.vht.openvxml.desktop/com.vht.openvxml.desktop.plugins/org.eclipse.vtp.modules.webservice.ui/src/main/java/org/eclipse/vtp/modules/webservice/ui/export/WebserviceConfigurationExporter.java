...
public class WebserviceConfigurationExporter implements IConfigurationExporter {
	...
	public void exportConfiguration(IFlowElement flowElement, Element actionElement) {
		...
		List<Element> managedConfigurationElementList = XMLUtilities.getElementsByTagName(
				baseConfigurationElement, "managed-config", true);
		for (Element managedConfigurationElement : managedConfigurationElementList) {
			if (WebserviceBindingManager.TYPE_ID.equals(managedConfigurationElement
					.getAttribute("type"))) {
				List<Element> serviceBindingElementList = XMLUtilities.getElementsByTagName(
						managedConfigurationElement, "service-binding", true);
				if (serviceBindingElementList.size() > 0) {
					...
					wsconfig.setSoapAction("static", serviceBindingElement
							.getAttribute("soap-action"));
				}
				List<Element> documentStructureElementList = XMLUtilities.getElementsByTagName(
						managedConfigurationElement, "input-document-structure", true);
				...
				List<Element> outputElementList = XMLUtilities.getElementsByTagName(
						managedConfigurationElement, "output-binding", true);
				if (outputElementList.size() > 0) {
					...
					wsconfig.setShouldProcess("true".equals(outputElement
							.getAttribute("should-process")));
					wsconfig.setOutputScriptText(XMLUtilities.getElementTextDataNoEx(outputElement,
							true));
				}
			}
		}
