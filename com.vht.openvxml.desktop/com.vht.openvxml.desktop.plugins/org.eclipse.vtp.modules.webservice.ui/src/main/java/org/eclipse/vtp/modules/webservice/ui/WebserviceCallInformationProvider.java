...
public class WebserviceCallInformationProvider extends PrimitiveInformationProvider {
	...
	public WebserviceCallInformationProvider(PrimitiveElement element) {
		...
		connectorRecords.add(new ConnectorRecord(element, "error.webservice",
	...
	/*
	 * public List getPropertiesPanels() { WebServiceCallSettingsStructure copy =
	 * (WebServiceCallSettingsStructure)settings.clone(); List ret = new ArrayList(); ret.add(new
	 * WebServiceCallTargetVariablePropertiesPanel(getElement(), copy)); ret.add(new
	 * WebServiceCallServicePropertiesPanel(getElement(), copy)); ret.add(new
	 * WebServiceCallInputPropertiesPanel(getElement(), copy)); ret.add(new
	 * WebServiceCallOutputPropertiesPanel(getElement(), copy)); return ret; }
	 */
	public List<Variable> getOutgoingVariables(String exitPoint, boolean localOnly) {
		List<Variable> ret = new ArrayList<Variable>();
		if (exitPoint.equals("Continue")) {
			WebserviceBindingManager manager = (WebserviceBindingManager) this.getElement()
					.getConfigurationManager(WebserviceBindingManager.TYPE_ID);
			Variable v = VariableHelper.constructVariable(manager.getOutputBinding()
					.getVariableName(), businessObjectSet, new FieldType(businessObjectSet
					.getBusinessObject("WSResponse")));
	...
	public void declareBusinessObjects() {
		...
		if (!foundWSResponse) {
			try {
				...
				((BusinessObject) wsResponse).write(getClass().getResourceAsStream(
						"/WSResponse.dod"));
			} catch (Exception e) {
				e.printStackTrace();
			}
