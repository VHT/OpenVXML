...
public class DesignParser {
	...
	public Design parseDesign(IDesignDocument designDocument, org.w3c.dom.Element designElement,
			ElementResolutionVisitor resolutionVisitor, boolean followExternalReferences) {
		return parseDesign(designDocument, null, designElement, resolutionVisitor,
				followExternalReferences);
	}

	public synchronized Design parseDesign(IDesignDocument designDocument, Design baseDesign,
			org.w3c.dom.Element designElement, ElementResolutionVisitor resolutionVisitor,
		...
		try {
			...
			if (elementsElementList.getLength() > 0) {
				...
				for (int i = 0; i < elementList.getLength(); i++) {
					...
					properties.setProperty("followExternalReferences", Boolean
							.toString(followExternalReferences));
					...
					element = ElementManager.getInstance().loadElement(elementType, elementId,
						...
						if (configurationList.getLength() > 0) {
							...
							for (int c = 0; c < managedConfigSectionList.getLength(); c++) {
								...
								ConfigurationManager manager = ConfigurationManagerRegistry
										.getInstance().getConfigurationManager(design,
												configSectionElement.getAttribute("type"));
								if (manager != null) {
									System.out.println("Loaded configuration manager "
											+ manager.getType() + " in "
											+ Long.toString(System.currentTimeMillis() - t));
									try {
										t = System.currentTimeMillis();
										manager.readConfiguration(configSectionElement);
										System.out.println("Read configuration  in "
												+ Long.toString(System.currentTimeMillis() - t));
			...
			if (connectorElementList.getLength() > 0) {
				NodeList connectorList = ((org.w3c.dom.Element) connectorElementList.item(0))
				...
				for (int i = 0; i < connectorList.getLength(); i++) {
					...
					if (origin == null || destination == null) { throw new RuntimeException(
							"orphaned connector: id=" + id + " origin=" + originId
