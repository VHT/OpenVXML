...
public class OldMediaToGenericConverter implements XMLConverter {
	...
	public void convert(Element element) throws ConversionException {
		...
		Element newManagedConfigElement = parentElement.getOwnerDocument().createElement(
				"managed-config");
		...
		NodeList promptBindingElementList = element.getElementsByTagNameNS(
				"http://eclipse.org/vtp/xml/configuration/media", "prompt-binding");
			...
			for (int pi = 0; pi < promptItemElementList.getLength(); pi++) {
				...
				if (itemBrand != null && itemLanguage != null && interactionId != null) {
					...
					if (newInteractionBindingElement == null) {
						newInteractionBindingElement = parentElement.getOwnerDocument()
					...
					if (newNamedBindingElement == null) {
						newNamedBindingElement = parentElement.getOwnerDocument().createElement(
								"named-binding");
					...
					Element newLanguageBindingElement = languageElements.get(interactionId
							+ bindingName + itemLanguage);
					if (newLanguageBindingElement == null) {
						newLanguageBindingElement = parentElement.getOwnerDocument().createElement(
								"language-binding");
						newNamedBindingElement.appendChild(newLanguageBindingElement);
						newLanguageBindingElement.setAttribute("language", itemLanguage);
						languageElements.put(interactionId + bindingName + itemLanguage,
								newLanguageBindingElement);
					}
					Element newBrandBindingElement = parentElement.getOwnerDocument()
					...
					Element bindingItemElement = parentElement.getOwnerDocument().createElement(
							"binding-item");
					newBrandBindingElement.appendChild(bindingItemElement);
					bindingItemElement.setAttribute("type",
					...
					for (int en = 0; en < promptBindingEntryList.getLength(); en++) {
						if (promptBindingEntryList.item(en).getNodeType() == Node.ELEMENT_NODE) {
							Element promptBindingEntryElement = (Element) promptBindingEntryList
									.item(en);
							bindingItemElement.appendChild(promptBindingEntryElement
									.cloneNode(true));
						}
					}
				}
		...
		NodeList grammarBindingElementList = element.getElementsByTagNameNS(
				"http://eclipse.org/vtp/xml/configuration/media", "grammar-binding");
					...
					for (int en = 0; en < grammarBindingEntryList.getLength(); en++) {
						if (grammarBindingEntryList.item(en).getNodeType() == Node.ELEMENT_NODE) {
							Element grammarBindingEntryElement = (Element) grammarBindingEntryList
									.item(en);
							bindingItemElement.appendChild(grammarBindingEntryElement
									.cloneNode(true));
						}
					}
				}
		...
		NodeList propertyBindingElementList = element.getElementsByTagNameNS(
				"http://eclipse.org/vtp/xml/configuration/media", "property-binding");
		for (int i = 0; i < propertyBindingElementList.getLength(); i++) {
			...
			for (int pi = 0; pi < propertyItemElementList.getLength(); pi++) {
				...
				if (itemBrand != null && itemLanguage != null && interactionId != null) {
					...
					Element propertyEntryItemElement = parentElement.getOwnerDocument()
