...
public class LegacySupportManager {
	...
	public LegacySupportManager() {
		...
		for (IConfigurationElement modelConverterExtension : modelConverterExtensions) {
			...
			Bundle contributor = Platform.getBundle(modelConverterExtension.getContributor().getName());
			try {
		...
		for (IConfigurationElement elementConverterExtension : elementConverterExtensions) {
			...
			Bundle contributor = Platform.getBundle(elementConverterExtension.getContributor().getName());
			try {
		...
		IConfigurationElement[] configurationManagerConverterExtensions = Platform.getExtensionRegistry()
				.getConfigurationElementsFor(configurationManagerConvertersExtensionId);
		for (IConfigurationElement configurationManagerConverterExtension : configurationManagerConverterExtensions) {
			...
			Bundle contributor = Platform.getBundle(configurationManagerConverterExtension.getContributor().getName());
			try {
				lec.converterClass = (Class<XMLConverter>) contributor.loadClass(className);
			} catch (ClassNotFoundException e) {
				e.printStackTrace();
