...
public class QuestionMediaDefaultsPanel implements IMediaDefaultPanel {
	...
	public Control createControls(Composite parent, boolean supportDefaults) {
		...
		if (supportDefaults) {
			lastStack.setValueControl(new ValueControl() {
				...
				@Override
				public void setValue(String value) {
					interdigitTimeoutSpinner.setSelection((value == null || value.equals("")) ? 3
							: Integer.parseInt(value));
				}
			});
		}
	...
	public void save() {
		if (settings.inheritanceSupported()) {
			...
			settings.getDefaultSetting(interactionType, elementType, "termination-character")
					.setValue(
		...
		if (settings.inheritanceSupported()) {
			...
			if (interdigitTimeoutSpinner != null) {
				...
				interdigitTimeoutSpinner.setSelection(interdigitTimeout == null
						|| interdigitTimeout.equals("") ? 3 : Integer.parseInt(interdigitTimeout));
			}
			...
			if (terminationCharacterCombo != null) {
				String terminationCharacter = settings.getDefaultSetting(interactionType,
						elementType, "termination-character").getValue();
