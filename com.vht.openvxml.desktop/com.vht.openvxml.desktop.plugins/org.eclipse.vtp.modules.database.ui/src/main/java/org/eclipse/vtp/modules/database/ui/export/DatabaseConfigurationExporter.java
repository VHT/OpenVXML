...
public class DatabaseConfigurationExporter implements IConfigurationExporter {
	...
	public void exportConfiguration(IFlowElement flowElement, Element actionElement) {
		...
		if (query.isResultArray()) query.setResultLimit(Integer.parseInt(settings
				.getAttribute("db-result-limit")));
		NodeList list = ((Element) settings.getElementsByTagNameNS(uri, "mappings").item(0))
		...
		for (int i = 0; i < list.getLength(); ++i) {
			...
			if ("0".equals(mappingElement.getAttribute("type"))) mapping
					.setType(DatabaseMappingConfiguration.TYPE_STATIC);
			else if ("1".equals(mappingElement.getAttribute("type"))) mapping
					.setType(DatabaseMappingConfiguration.TYPE_COLUMN);
			else mapping.setType(DatabaseMappingConfiguration.TYPE_NONE);
			if (mapping.getType() != DatabaseMappingConfiguration.TYPE_NONE) mapping
					.setValue(mappingElement.getAttribute("value"));
		...
		for (int i = 0; i < list.getLength(); ++i) {
			...
			if ("0".equals(criteriumElement.getAttribute("type"))) criteria
					.setType(DatabaseCriteriaConfiguration.TYPE_STATIC);
			else if ("1".equals(criteriumElement.getAttribute("type"))) criteria
					.setType(DatabaseCriteriaConfiguration.TYPE_VARIABLE);
			else criteria.setType(DatabaseCriteriaConfiguration.TYPE_NONE);
			if (criteria.getType() != DatabaseCriteriaConfiguration.TYPE_NONE) {
				if ("1".equals(criteriumElement.getAttribute("comp"))) criteria
						.setComparison(DatabaseCriteriaConfiguration.COMPARISON_NOT_EQUAL);
				else if ("2".equals(criteriumElement.getAttribute("comp"))) criteria
						.setComparison(DatabaseCriteriaConfiguration.COMPARISON_LESS_THAN);
				else if ("3".equals(criteriumElement.getAttribute("comp"))) criteria
						.setComparison(DatabaseCriteriaConfiguration.COMPARISON_LESS_THAN_OR_EQUAL);
				else if ("4".equals(criteriumElement.getAttribute("comp"))) criteria
						.setComparison(DatabaseCriteriaConfiguration.COMPARISON_GREATER_THAN);
				else if ("5".equals(criteriumElement.getAttribute("comp"))) criteria
						.setComparison(DatabaseCriteriaConfiguration.COMPARISON_GREATER_THAN_OR_EQUAL);
				else criteria.setComparison(DatabaseCriteriaConfiguration.COMPARISON_EQUAL);
