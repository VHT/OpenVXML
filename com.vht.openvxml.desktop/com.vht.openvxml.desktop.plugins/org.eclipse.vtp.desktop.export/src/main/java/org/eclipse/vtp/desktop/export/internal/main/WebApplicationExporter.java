...
public class WebApplicationExporter {
	...
	private static final List<String> REQUIRED_BUNDLES = Collections.unmodifiableList(Arrays
			.asList(new String[] { "org.eclipse.osgi", "org.eclipse.osgi.services",
					"org.eclipse.core.contenttype", "org.eclipse.core.jobs",
					"org.eclipse.core.runtime", "org.eclipse.core.runtime.compatibility.auth",
					"org.eclipse.equinox.app", "org.eclipse.equinox.common",
					"org.eclipse.equinox.http.servlet", "org.eclipse.equinox.http.servletbridge",
					"org.eclipse.equinox.preferences", "org.eclipse.equinox.registry",
					"org.eclipse.equinox.servletbridge", "org.eclipse.update.configurator",
					"javax.activation", "javax.mail", "javax.xml.soap", "javax.xml.rpc",
					"javax.wsdl", "org.apache.ant", "org.apache.axis",
					"org.apache.commons.discovery", "org.apache.commons.io",
					"org.apache.commons.fileupload", "org.apache.commons.logging",
					"org.apache.commons.pool", "org.apache.log4j", "org.apache.xerces",
					"org.apache.xml.resolver", "org.mozilla.javascript",
					"org.eclipse.vtp.framework.core", "org.eclipse.vtp.framework.engine",
					"org.eclipse.vtp.framework.spi", "org.eclipse.vtp.framework.util",
					"org.apache.commons.codec", "org.apache.commons.lang" }));

	private static final List<String> OPTIONAL_BUNDLES = Collections.unmodifiableList(Arrays
			.asList(new String[] { "javax.xml", "org.apache.xml.serializer" }));

	private static final List<String> EXTENSION_POINTS = Collections.unmodifiableList(Arrays
			.asList(new String[] { "org.eclipse.vtp.framework.core.actions",
	...
	public final synchronized boolean export(File archive,
			Collection<WorkflowExporter> workflowProjects, Collection<MediaExporter> mediaProjects,
			boolean separateMedia, IProgressMonitor monitor) throws Exception {
	...
	private boolean run() throws Exception {
		...
		for (WorkflowExporter project : workflowExporters) {
			if (monitor.isCanceled()) { return false; }
			monitor.setTaskName(String.format("Exporting workflow project %s...", project
					.getProject().getName()));
		...
		for (MediaExporter project : mediaExporters) {
			if (monitor.isCanceled()) { return false; }
			monitor.setTaskName(String.format("Exporting media project %s...", project.getProject()
					.getName()));
			if (!separateMedia) {
				exportMedia(project);
			} else {
	...
	private void constructRuntime() {
		...
		for (String extensionPoint : EXTENSION_POINTS) {
			for (IExtension extension : extensionRegistry.getExtensionPoint(extensionPoint)
					.getExtensions()) {
		...
		for (Bundle bundle : ExportCore.getBundles()) {
			...
			if (exporter == null) {
				...
				System.out.println("Multiple bundle versions: " + symbolicName + " current: "
						...
						+ bundle.getHeaders().get(Constants.BUNDLE_VERSION));
				Version oldVersion = new Version((String) exporter.getHeaders().get(
	...
	private void exportFramework() throws Exception {
		...
		PrintStream frameworkConfig = new PrintStream(output
				.write("WEB-INF/eclipse/configuration/config.ini"));
		...
		document.appendChild(ConfigurationDictionary.saveAll(document, dictionaries
				.toArray(new ConfigurationDictionary[dictionaries.size()])));
	...
	private void exportExternalMedia(MediaExporter project) throws Exception {
		FilenameFilter filter = new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				if (name.endsWith(".xml") || name.endsWith(".grxml") || name.endsWith(".gram")
						|| name.endsWith(".grammar") || name.endsWith(".regex")
						|| name.endsWith(".regx")) { return true; }
				File child = new File(dir, name);
				if (child.exists() && child.isDirectory()) { return true; }
				return false;
			}
		...
		pluginXmlDoc.appendChild(pluginXmlDoc.createProcessingInstruction("eclipse",
				"version=\"3.2\""));
		...
		indexMedia(fileIndex, project.getMediaProject().getMediaLibrariesFolder()
				.getUnderlyingFolder());
	...
	private void exportWorkflow(WorkflowExporter project) throws Exception {
		...
		Map<String, String> formatterIDsByLanguage = new HashMap<String, String>(mediaProjects
				.size());
		...
		for (Map.Entry<String, MediaExporter> entry : mediaProjects.entrySet()) {
			...
			resourceManagerIDsByLanguage.put(entry.getKey(), entry.getValue().getProject()
					.getName());
		}
		...
		for (IDesignDocument document : project.getDesignDocuments()) {
			callDesigns.add(new DesignReference(document, documentBuilder.parse(document
					.getUnderlyingFile().getLocation().toFile().getCanonicalFile())));
		}
		Document definition = new DefinitionBuilder(callDesigns
				.toArray(new DesignReference[callDesigns.size()]), documentBuilder, project
				.getWorkflowProject(), formatterIDsByLanguage, resourceManagerIDsByLanguage,
				project.getLanguageMapping()).getDefinition();
		Element servicesElement = (Element) definition.getDocumentElement().getElementsByTagNameNS(
				"http://eclipse.org/vtp/xml/framework/engine/process-definition", "services").item(
				0);
		...
		transformer.transform(new DOMSource(definition), new XMLWriter(fileEntryStream)
				.toXMLResult());
	...
	private void finalizeWebApplication() throws Exception {
		...
		output.writeURL("WEB-INF/lib/servletbridge.jar", getClass().getResource(
				"/servletbridge.jar"));
	}
