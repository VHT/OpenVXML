...
public class InitialAssignmentConfigurationExporter implements IConfigurationExporter {
	...
	public void exportConfiguration(IFlowElement flowElement, Element actionElement) {
		...
		Element configElement = (Element) flowElement.getConfiguration().getElementsByTagNameNS(
				uri, "custom-config").item(0);
		...
		Element icElement = actionElement.getOwnerDocument().createElementNS(
				IDefinitionBuilder.NAMESPACE_URI_COMMON, "common:initial"); //$NON-NLS-1$
		ic.save(icElement);
		actionElement.appendChild(icElement);
		ConfigurationExportHelper.configureAssignmentAction(flowElement.getConfiguration(),
				actionElement);
	}
