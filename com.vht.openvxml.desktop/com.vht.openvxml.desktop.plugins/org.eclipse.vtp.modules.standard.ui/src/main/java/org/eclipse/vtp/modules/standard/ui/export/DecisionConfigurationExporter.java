...
public class DecisionConfigurationExporter implements IConfigurationExporter {
	...
	public void exportConfiguration(IFlowElement flowElement, Element actionElement) {
		...
		if (comparisonList.getLength() > 0) {
			...
			config.setType(BranchConfiguration.COMPARISON_TYPE_LESS_THAN_OR_EQUAL);
			...
			config.setType(BranchConfiguration.COMPARISON_TYPE_GREATER_THAN_OR_EQUAL);
			...
			} else config.setLeftVariableValue(comparison.getAttribute("left-value")); //$NON-NLS-1$
			if ("expression".equalsIgnoreCase( //$NON-NLS-1$
					comparison.getAttribute("right-type"))) //$NON-NLS-1$
			{
				config.setRightExpressionValue(comparison.getAttribute("right-value"), //$NON-NLS-1$
						"JavaScript"); //$NON-NLS-1$
				config.setRightSecured(Boolean.parseBoolean(comparison
						.getAttribute("right-secured")));
			} else config.setRightVariableValue(comparison.getAttribute("right-value")); //$NON-NLS-1$
