...
public class BranchPropertiesPanel extends DesignElementPropertiesPanel {
	 ...
	 * (non-Javadoc)
	 * @see
	 * org.eclipse.vtp.desktop.media.core.MediaConfigurationScreen#createControls(org.eclipse.swt
	 * .widgets.Composite)
	 */
	public void createControls(Composite parent) {
		...
		final TableViewer branchTableViewer = new TableViewer(tableComp, SWT.FULL_SELECTION
		...
		TableViewerColumn branchNameViewerColumn = new TableViewerColumn(branchTableViewer,
				SWT.NONE);
		...
		layout.setColumnData(branchNameColumn, new ColumnWeightData(2,
				ColumnWeightData.MINIMUM_WIDTH, true));

		TableViewerColumn branchValueViewerColumn = new TableViewerColumn(branchTableViewer,
				SWT.NONE);
		...
		layout.setColumnData(branchValueColumn, new ColumnWeightData(7,
				ColumnWeightData.MINIMUM_WIDTH, true));
		...
		branchTableViewer.addDoubleClickListener(new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent event) {
				if (!branchTableViewer.getSelection().isEmpty()) {
					BranchConfigurationDialog bcd = new BranchConfigurationDialog(Display
							.getCurrent().getActiveShell(), branches);
					Branch br = (Branch) ((IStructuredSelection) branchTableViewer.getSelection())
							.getFirstElement();
					bcd.setValue(br);
					if (bcd.open() == SWT.OK) {
		...
		addButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				BranchConfigurationDialog bcd = new BranchConfigurationDialog(Display.getCurrent()
						.getActiveShell(), branches);
		...
		upButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (!branchTableViewer.getSelection().isEmpty()) {
					Branch branch = (Branch) ((IStructuredSelection) branchTableViewer
		...
		branchTableViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (selection == null || selection.isEmpty()) {
					...
					if (branches.indexOf(((IStructuredSelection) selection).getFirstElement()) == branches
							.size() - 1) {
						downButton.setEnabled(false);
					}
				}
	...
	public class BranchTableLabelProvider implements ITableLabelProvider {
		...
		public Image getColumnImage(Object element, int columnIndex) {
			...
			if (columnIndex == 0 && branch.isSecure()) return Activator.getDefault()
					.getImageRegistry().get("ICON_LOCK");
			return null;
		}
		...
		public String getColumnText(Object element, int columnIndex) {
			...
			if (columnIndex == 1) return br.getName();
