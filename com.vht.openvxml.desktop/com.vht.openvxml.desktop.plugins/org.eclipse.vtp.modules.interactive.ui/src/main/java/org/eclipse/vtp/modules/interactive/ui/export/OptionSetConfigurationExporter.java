...
public class OptionSetConfigurationExporter implements IConfigurationExporter {
	...
	public void exportConfiguration(IFlowElement flowElement, Element actionElement) {
		SelectionRequestConfiguration config = new SelectionRequestConfiguration(
				ContentLoadingManager.getInstance(), InputLoadingManager.getInstance());
		...
		List<Element> legacyChoicesElement = XMLUtilities.getElementsByTagName(customConfig,
				"choices", true);
		if (legacyChoicesElement.size() > 0) // legacy conversion
		{
			List<Element> choiceElements = XMLUtilities.getElementsByTagName(legacyChoicesElement
					.get(0), "choice", true);
			for (Element choiceElement : choiceElements) {
				...
				String prefix = name + "-";
				MediaConfiguration choiceMedia = new MediaConfiguration(ContentLoadingManager
				...
				for (int j = 0; j < outputNames.length; ++j) {
					...
					OutputConfiguration output = media.getOutputConfiguration(outputNames[j]);
					media.setOutputConfiguration(outputNames[j], null);
					choiceMedia.setOutputConfiguration(outputNames[j].substring(prefix.length()),
							output);
				}
				...
				for (int j = 0; j < inputNames.length; ++j) {
					...
					choiceMedia.setInputConfiguration(inputNames[j].substring(prefix.length()),
							input);
				}
			...
			List<Element> brandOrderElements = XMLUtilities.getElementsByTagName(customConfig,
					"brand-order", true);
			...
			for (Element brandOrder : brandOrderElements) {
				...
				config.setBrandedChoices(brand, entries.values()
						.toArray(new String[entries.size()]));
				entries.clear();
			}
		} else {
			List<Element> managedConfigs = XMLUtilities.getElementsByTagName(flowElement
					.getConfiguration(), "managed-config", true);
			...
			for (Element element : managedConfigs) {
				...
				if (!"org.eclipse.vtp.configuration.menuchoice"
						.equals(element.getAttribute("type"))) {
					continue;
				}
				Element managedConfig = element;
				List<Element> choicesElements = XMLUtilities.getElementsByTagName(managedConfig,
						"choices", true);
				if (choicesElements.size() == 0) {
					continue;
				}
				List<Element> choiceElements = XMLUtilities.getElementsByTagName(choicesElements
						.get(0), "choice", true);
				for (Element choiceElement : choiceElements) {
					String name = choiceElement.getAttribute("name"); //$NON-NLS-1$
					String script = null;
					...
					for (int j = 0; j < propertyNames.length; ++j) {
						...
						PropertyConfiguration property = media
								.getPropertyConfiguration(propertyNames[j]);
						media.setPropertyConfiguration(propertyNames[j], null);
						choiceMedia.setPropertyConfiguration(propertyNames[j].substring(prefix
								.length()), property);
					}
					String[] outputNames = media.getOutputConfigurationNames();
					for (int j = 0; j < outputNames.length; ++j) {
						...
						OutputConfiguration output = media.getOutputConfiguration(outputNames[j]);
						media.setOutputConfiguration(outputNames[j], null);
						choiceMedia.setOutputConfiguration(outputNames[j]
								.substring(prefix.length()), output);
					}
					String[] inputNames = media.getInputConfigurationNames();
					for (int j = 0; j < inputNames.length; ++j) {
						if (!inputNames[j].startsWith(prefix)) {
				...
				config.setMediaConfiguration(media);
				List<Element> brandOrderElements = XMLUtilities.getElementsByTagName(managedConfig,
						"brand-order", true);
				Map<Integer, String> entries = new TreeMap<Integer, String>();
				for (Element brandOrder : brandOrderElements) {
					String brand = brandOrder.getAttribute("brand"); //$NON-NLS-1$
