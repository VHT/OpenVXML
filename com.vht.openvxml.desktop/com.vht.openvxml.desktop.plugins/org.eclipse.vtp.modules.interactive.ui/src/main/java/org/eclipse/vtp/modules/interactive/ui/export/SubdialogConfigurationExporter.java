...
public class SubdialogConfigurationExporter implements IConfigurationExporter {
	...
	public void exportConfiguration(IFlowElement flowElement, Element actionElement) {
		...
		NodeList list = ((Element) custom.getElementsByTagNameNS(uri, "inputs").item(0))
				.getElementsByTagNameNS(uri, "input");
		for (int i = 0; i < list.getLength(); ++i) {
			Element elem = (Element) list.item(i);
			config.setInput(elem.getAttribute("name"), "1".equals(elem.getAttribute("type")), elem
					.getAttribute("value"));
		}
		...
		for (int i = 0; i < list.getLength(); ++i) {
			...
			config.setURLParameter(elem.getAttribute("name"),
					"1".equals(elem.getAttribute("type")), elem.getAttribute("value"));
		}
