...
public class ApplicationEditor extends EditorPart
	implements
	ControllerListener,
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		...
		try {
			project = WorkflowCore.getDefault().getWorkflowModel().convertToWorkflowProject(
			...
			partListener = new IPartListener() {
				...
				public void partActivated(IWorkbenchPart part) {
							...
							new Action("Copy") {
								...
								public void run() {
									final SelectionStructure selection = currentCanvas.renderedCanvas
											.getSelection();
									if (selection.getPrimarySelection() instanceof ConnectorFrame) { return; }
					...
					site.getActionBars().setGlobalActionHandler(ActionFactory.PASTE.getId(),
							new Action("Paste") {
								@Override
								public void run() {
									...
									if (text != null) {
										try {
											...
											PartialDesignDocument pdd = new PartialDesignDocument(
													designDocument, null, document);
											PasteOperation po = new PasteOperation(pdd,
													currentCanvas);
					...
					site.getActionBars().setGlobalActionHandler(ActionFactory.PRINT.getId(),
							new Action("Print") {
								...
								public void run() {
									...
									if (printerData != null) {
										...
										for (int i = printerData.startPage - 1; i < Math.min(
												printerData.endPage, designs.size()); i++) {
											CanvasRecord cr = designs.get(i);
											RenderedModel renderedModel = cr.renderedCanvas;
											printer.startPage();
											Point dpi = printer.getDPI();
											System.out.println("printer dpi: " + dpi.x + ", "
													+ dpi.y);
											...
											if (renderedModel.getUIModel().getOrientation() != printerOrientation) {
												transform.translate(renderedModel.getUIModel()
														.getHeight() / 2, renderedModel
														.getUIModel().getHeight() / 2);
												transform.rotate(90f);
												transform.translate(-1f
														* renderedModel.getUIModel().getHeight()
														/ 2, -1f
														* renderedModel.getUIModel().getHeight()
														/ 2);
											}
											...
											gc.setLineWidth(1);
											renderedModel.paintCanvas(gc, resourceMap,
	...
	private void copySelectionToClipboard(final SelectionStructure selection) {
		...
		try {
			...
			if (!ApplicationEditor.this.currentCanvas.renderedCanvas.getUIModel().equals(
					designDocument.getMainDesign())) {
				rootElement.setAttribute("dialog-only", "true");
				writer.writeDesign(rootElement, (Design) currentCanvas.renderedCanvas.getUIModel(),
						selectionFilter);
			} else {
	...
	public void createPartControl(Composite parent) {
		...
		File workingLocation = fileInput.getFile().getProject().getWorkingLocation(
				"org.eclipse.vtp.desktop.model.core").toFile();
		File iconPath = new File(workingLocation, fileInput.getFile().getProjectRelativePath()
				.toString()
				+ "/");
		...
		for (CanvasRecord cr : designs) {
			...
			if (renderedModel.getUIModel().getDocument().getDesignThumbnail(
				...
				try {
					...
					File iconFile = new File(iconPath, cr.renderedCanvas.getUIModel().getDesignId()
							+ ".jpg");
	...
	public Object getAdapter(Class adapter) {
		...
		if (adapter.equals(ModelNavigationListener.class)) { return this; }
		if (adapter.equals(IResource.class)) { return ((FileEditorInput) getEditorInput())
				.getFile(); }
		if (adapter.equals(IDesign.class)) { return currentCanvas.renderedCanvas.getUIModel(); }
	...
	private class CanvasRecord implements RenderedModelListener {
		...
		public CanvasRecord(RenderedModel renderedCanvas) {
			...
			canvasFrame = ThemeManager.getDefault().getDefaultTheme().createCanvasFrame(
					renderedCanvas.getUIModel());
		...
		public void renderedModelFormatChanged(RenderedModel renderedCanvas) {
			...
			sc.setMinSize(width + canvasFrame.getInsets().x + canvasFrame.getInsets().width, height
		...
		public class CanvasUndoContext implements IUndoContext {
			...
			public boolean matches(IUndoContext context) {
				if (context instanceof CanvasUndoContext) { return getId().equals(
						((CanvasUndoContext) context).getId()); }
				return false;
			}
		}
	}
