...
public class DialogElementConfigurationExporter implements IConfigurationExporter {
	...
	public String getTargetId(IFlowElement flowElement, Element afterTransitionElement) {
		IFlowModel dialogModel = flowElement.getModel().getDefinitionBuilder().getDialogModel(
				flowElement.getId());
		return dialogModel == null ? null : dialogModel.getEntries().isEmpty() ? null : dialogModel
				.getEntries().iterator().next().getTargetID(afterTransitionElement);
	}
